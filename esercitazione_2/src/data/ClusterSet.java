...
public class ClusterSet {
	...
	void inizializeCentroids(Data data){
		...
		for(int i=0;i<centroidIndex.length;i++){
			...
			add(new Cluster(centroidI));//memorizza il centroide
		}
	}
		
	
	/**
	 * NON SO SE FUNZIONA. NON CI SONO DI SICURO ERRORI DI SINTASSI
	 * 
	 * Logica usata
	...
	Cluster[] nearestCluster(Tuple tuple)
	{
		...
		for(int i=0;i<tuple.getLength();i++)
		{
			...
			//una tupla
			if(distanza>(tuple.getDistance(C[i].getCentroid())))
				distanza=tuple.getDistance(C[i].getCentroid());
		}
		return C;
	}
}	
